...
from pentool.report.styles import C, SEV_COLOR, SEV_BG, build_styles
...
def _score_para(score, styles: dict) -> Paragraph:
    ...
    if score >= 9.0:   col = "#FF3B3B"
    elif score >= 7.0: col = "#FF8C00"
    elif score >= 4.0: col = "#FFD600"
    else:              col = "#4CAF50"
    return Paragraph(f'<font color="{col}"><b>{score:.1f}</b></font>', styles["table_cell"])
...
class ReportGenerator:
    ...
    def _cover_page(self) -> list:
        ...
        critical  = sum(
            sum(1 for c in m.get("cves", []) if c.get("cvss_v3_severity") == "CRITICAL")
        ...
        for label, value in meta_lines:
            line = f'<font color="#00D4AA"><b>{label} :</b></font>  ' \
        ...
        story.append(Paragraph("Projet ISEN — Réalisé avec Pentool v0.1.0", S["cover_meta"]))
    ...
    def _executive_summary(self) -> list:
        ...
        story.append(HRFlowable(width="100%", thickness=0.5, color=C.BORDER, spaceAfter=4*mm))
        ...
        if max_score >= 9.0:   risk_label, risk_col = "CRITIQUE",  "#FF3B3B"
        elif max_score >= 7.0: risk_label, risk_col = "ÉLEVÉ",     "#FF8C00"
        elif max_score >= 4.0: risk_label, risk_col = "MODÉRÉ",    "#FFD600"
        elif max_score > 0:    risk_label, risk_col = "FAIBLE",    "#4CAF50"
        else:                  risk_label, risk_col = "INCONNU",   "#9E9E9E"
        ...
        n_crit    = sum(
            ...
            for m in self._cve_data)
        n_high    = sum(
            sum(1 for c in m.get("cves", []) if c.get("cvss_v3_severity") == "HIGH")
            for m in self._cve_data)
        n_medium  = sum(
            sum(1 for c in m.get("cves", []) if c.get("cvss_v3_severity") == "MEDIUM")
            for m in self._cve_data)
        fuzz_found = len([
            r for r in self._fuzz_data.get("results", [])
        ...
        summary_data = [
            ...
            [Paragraph("Score de risque global", S["table_cell"]),
             Paragraph(f'<font color="{risk_col}"><b>{max_score:.1f} / 10</b></font>', S["table_cell"]),
             Paragraph(f'<font color="{risk_col}"><b>{risk_label}</b></font>', S["table_cell"])],

            [Paragraph("Ports ouverts", S["table_cell"]),
             Paragraph(str(len(open_svcs)), S["table_cell"]),
             Paragraph(", ".join(str(s["port"]) for s in open_svcs[:8]), S["table_cell"])],

            [Paragraph("CVE CRITICAL", S["table_cell"]),
             Paragraph(f'<font color="#FF3B3B"><b>{n_crit}</b></font>', S["table_cell"]),
             Paragraph("Score CVSS >= 9.0", S["table_cell"])],

            [Paragraph("CVE HIGH", S["table_cell"]),
             Paragraph(f'<font color="#FF8C00"><b>{n_high}</b></font>', S["table_cell"]),
             Paragraph("Score CVSS 7.0–8.9", S["table_cell"])],

            [Paragraph("CVE MEDIUM", S["table_cell"]),
             Paragraph(f'<font color="#FFD600"><b>{n_medium}</b></font>', S["table_cell"]),
            ...
            [Paragraph("Répertoires HTTP trouvés", S["table_cell"]),
             ...
             Paragraph(f'{self._fuzz_data.get("total_tested", 0)} chemins testés', S["table_cell"])],
        ]
    ...
    def _section_ports(self) -> list:
        ...
        story.append(HRFlowable(width="100%", thickness=0.5, color=C.BORDER, spaceAfter=3*mm))
        ...
        story.append(Paragraph(f"<b>Commande nmap :</b> <font name='Courier'>{scan_args}</font>", S["body_muted"]))
    ...
    def _section_dns(self) -> list:
        ...
        if whois:
            ...
            rows = [[Paragraph("Champ", S["table_header"]),
                     Paragraph("Valeur", S["table_header"])]]
    ...
    def _section_fuzz(self) -> list:
        ...
        story.append(Paragraph(f"{n + 1}. Fuzzing de Répertoires HTTP", S["section_title"]))
        story.append(HRFlowable(width="100%", thickness=0.5, color=C.BORDER, spaceAfter=3*mm))
        ...
        interesting = [r for r in results if r.get("status_code") in
                       (200, 201, 204, 301, 302, 403, 401)]
        ...
        if not interesting:
            story.append(Paragraph("Aucune ressource accessible découverte.", S["body"]))
            story.append(PageBreak())
            return story
    ...
    def _section_cve(self) -> list:
        ...
        sec = 2
        if self._dns_data:  sec += 1
        if self._fuzz_data: sec += 1
        sec += 1

        story.append(Paragraph(f"{sec}. Vulnérabilités CVE Identifiées", S["section_title"]))
        story.append(HRFlowable(width="100%", thickness=0.5, color=C.BORDER, spaceAfter=3*mm))
        ...
        rows = [header]
        for m in self._cve_data:
            n_crit = sum(1 for c in m.get("cves",[]) if c.get("cvss_v3_severity")=="CRITICAL")
            n_high = sum(1 for c in m.get("cves",[]) if c.get("cvss_v3_severity")=="HIGH")
            scores = [c.get("cvss_v3_score") or c.get("cvss_v2_score") or 0 for c in m.get("cves",[])]
            ...
            rows.append([
                ...
                Paragraph(f'<font color="#FF3B3B"><b>{n_crit}</b></font>' if n_crit else "0", S["table_cell"]),
                Paragraph(f'<font color="#FF8C00"><b>{n_high}</b></font>'  if n_high else "0", S["table_cell"]),
        ...
        for m in self._cve_data:
            ...
            cve_rows = [cve_header]
            for c in cves:
                sev      = c.get("cvss_v3_severity", "UNKNOWN")
                score_v  = c.get("cvss_v3_score") or c.get("cvss_v2_score")
                ...
                cve_rows.append([
                    Paragraph(f'<font color="#00D4AA">{cve_id}</font>', S["table_mono"]),
                    Paragraph(f'<font color="{hex_c}"><b>{sev}</b></font>', S["table_cell"]),
    ...
    def _build_recommendations(self) -> list[tuple[str, str, str]]:
        ...
        recs = []
        services = {s.get("service","") for s in self._port_data.get("services",[]) if s.get("state")=="open"}
        open_ports = [s.get("port") for s in self._port_data.get("services",[]) if s.get("state")=="open"]

        # CVE critiques
        for m in self._cve_data:
            for c in m.get("cves", []):
                if c.get("cvss_v3_severity") == "CRITICAL":
        ...
        critical_paths = [
            r.get("path","") for r in self._fuzz_data.get("results",[])
