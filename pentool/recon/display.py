"""
Affichage Rich des résultats de reconnaissance.
...
from pentool.recon.port_scanner import HostResult
...
def print_port_scan(result: HostResult) -> None:
    ...
    console.print(Panel("\n".join(host_lines), title="[cyan]Hôte[/cyan]",
    ...
    table = Table(show_header=True, header_style="bold cyan", box=box.ROUNDED,
                  border_style="dim", row_styles=["", "dim"], expand=False)
    ...
    for svc in open_svcs:
        table.add_row(
            str(svc.port), svc.protocol, _state_badge(svc.state),
            svc.service, svc.product or "—", svc.version or "—",
            svc.extrainfo or "—",
        )
    console.print(table)
    ...
    fp_table = Table(box=box.SIMPLE, show_header=False, padding=(0, 2))
    fp_table.add_column(style="bold white", justify="right")
    fp_table.add_column(style="cyan")
    fp_table.add_column(style="dim")
    for svc in open_svcs:
...
def print_dns_enum(result: DNSEnumResult) -> None:
    ...
    if result.records:
        t = Table(title="[bold]Enregistrements DNS[/bold]", box=box.ROUNDED,
                  border_style="dim", header_style="bold cyan", expand=False)
        t.add_column("Type",   style="bold yellow", width=8)
        t.add_column("Valeur", style="white",       width=50)
        t.add_column("TTL",    style="dim",         width=10)
        for rec in result.records:
            t.add_row(rec.rtype, rec.value, str(rec.ttl))
        console.print(t)
    else:
    ...
    if result.subdomains:
        t = Table(title=f"[bold]Sous-domaines découverts ({len(result.subdomains)})[/bold]",
                  box=box.ROUNDED, border_style="dim", header_style="bold cyan", expand=False)
        t.add_column("Sous-domaine", style="green",  width=40)
        t.add_column("IP",           style="yellow", width=20)
        t.add_column("CNAME",        style="dim",    width=30)
        for sub in result.subdomains:
            t.add_row(sub.subdomain, sub.ip, sub.cname or "—")
        console.print(t)
    ...
    if result.whois_data:
        ...
        t = Table(title="[bold]WHOIS[/bold]", box=box.ROUNDED,
                  border_style="dim", header_style="bold cyan", expand=False)
        t.add_column("Champ",  style="bold yellow", width=20)
        t.add_column("Valeur", style="white",       width=50)
        for key, label in whois_fields:
...
def print_fuzz_results(result) -> None:
    ...
    console.print(Panel("\n".join(summary), title="[cyan]Fuzzing HTTP[/cyan]",
                        border_style="cyan", expand=False))
    ...
    if crits:
        console.print(f"\n[danger]⚠ {len(crits)} ressource(s) CRITIQUE(S) :[/danger]")
        for r in crits:
    ...
    for r in found:
        ...
        table.add_row(
            _SEVERITY_STYLE.get(r.severity, r.severity),
